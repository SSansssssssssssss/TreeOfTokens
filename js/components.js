...
// data = a function returning the content (actually HTML)
	...
	//Custom component for Vue (I don't how to do this heh) [Data = bg img, tooltip, display (can be a function)]
	Vue.component('effect', {
		props: ['layer', 'data'],
		template: `
			<div>
				<div v-bind:style="{'background-image': 'url('+data[0]+')', 'display': data[2]}" v-bind:class="{ [layer]: true, tooltipBox: true, effect: true }">
					<tooltip v-if="Array.isArray(data)" :text="data[1]"></tooltip>
				</div>
			</div>
		`
	})
